...
class ContentsListView extends Component {
  ...
  getContents = async () => {
    const contents = await this.context.actions.getContentsRepresentation1();
    this.setState({
      contents: contents,
    });
  };
  ...
  render() {
    return this.state.contents.map((contents, index) => {
      return (
        <div key={index}>
          ...
          <img
            src={`http://localhost:8080/${contents.imageUrl}`}
            width="250"
            height="250"
            alt="coverimg"
          />
